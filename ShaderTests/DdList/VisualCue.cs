...
public class VisualCue
{
    ...
    public void Draw(int itemIndex)
    {
        ...
        nint hdc = Win32.GetDC(nint.Zero); // Get device context.
        Win32.SetROP2(hdc, Win32.R2_NOT); // Switch to inverted mode.
        Win32.MoveToEx(hdc, l1p1.X, l1p1.Y, nint.Zero);
        Win32.LineTo(hdc, l1p2.X, l1p2.Y);
        Win32.MoveToEx(hdc, l2p1.X, l2p1.Y, nint.Zero);
        Win32.LineTo(hdc, l2p2.X, l2p2.Y);
        Win32.ReleaseDC(nint.Zero, hdc); // Release device context.
        _index = itemIndex;
    }
}
